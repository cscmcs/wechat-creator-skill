...
[![MCP Supported](https://img.shields.io/badge/MCP-11%20Tools%20Ready-blueviolet.svg)](https://modelcontextprotocol.io/)
...
- 遵循 2.35:1 官方比例与少字化原则规划封面海报及正文视觉，并通过 MCP 自动同步入库与推送至公众号**官方草稿箱**；
- 预置 8 套精选内联主题 + 4 套杂志高定排版（松烟墨绿、手泽深赭、金石灰金、霓灰极光），支持出版级微下划线与红绿对比卡；
- 深度支持 **Model Context Protocol (MCP)**：Agent 写完自动入库平台后台，用户可随时在网页端可视化微调，也可一键推至微信公众平台。
...
- **MCP 平台双向闭环与官方草稿箱同步**：
  ...
  - 授权后调用 `create_wechat_draft` 接口安全直达微信公众平台草稿箱。
...
| `save_article` | Phase 7 | **核心必选动作**：将撰写排版完毕的文章自动存入用户个人平台文章库 |
| `create_wechat_draft` | Phase 7 | 将成稿（含排版 HTML 与封面）安全推送至绑定的微信公众平台官方草稿箱 |
| `get_writer_profile` | Phase 2 | 读取用户的个人写作画像（语气、人称、句式习惯、禁忌表达），实现千人千面 |
| `search_materials` | Phase 1 | 检索用户在平台上积累的私有事实素材、案例卡片与灵感备忘 |
| `upload_wechat_image` | Phase 6 | 将配图上传至微信官方素材 CDN，返回永久文章插图 URL |
...
| **草稿同步** | 自动存入平台文章库，可一键推草稿箱 | 自动存入平台文章库，可一键推草稿箱 |
...
> 要求：清单体结构，短句断行，去掉所有空洞大词，使用【科技蓝】排版，完成后自动调用 save_article 保存至后台，并推送到微信草稿箱。”
...
[阶段 1: 事实查证] -> [阶段 2: 结构写作] -> [阶段 3: 去AI味责任编辑]
         |                     |                       |
[阶段 7: MCP 直推与草稿] <- [阶段 6: 封面设计] <- [阶段 5: 内联排版] <- [阶段 4: 5类真实标题]
```
...
6. **阶段 6：2.35:1 官方封面海报设计 (`references/cover.md`)**：900×383 黄金规格，少字化构图建议；支持 MCP `upload_wechat_cover`。
7. **阶段 7：MCP 自动直推平台与微信官方草稿箱同步**：自动调用 `save_article` 持久化至平台数据库，支持后续在网页端出版级微调；调用 `create_wechat_draft` 推至官方后台。
...
### Q: 技能会自动把文章直接发给所有粉丝吗？
**A**：**绝对不会！** 系统的设计底线是仅调用官方草稿箱接口（`create_wechat_draft`），**绝不执行公开发布**。文章保存至草稿箱后，必须由公众号管理员在微信后台最终扫码确认才能发布。

### Q: Agent 写完文章后如何与平台网站互动？
**A**：通过 MCP 的 `save_article` 工具，Agent 会自动将排版渲染好的 Markdown 与微信 HTML 一同存入平台数据库。用户可以在平台 Web 界面上打开该文章，使用**出版级排版定制器**拖拽调色、微调字号或增减对比卡，然后再一键推送到微信公众平台。
