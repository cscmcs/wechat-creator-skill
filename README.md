...
意图识别 (Intent Router)
   ...
   ├─► 阶段 3：确定角度和结构
   ├─► 阶段 4：Draft 初稿
   ├─► 阶段 5：Author Trace 作者痕迹检查
   ├─► 阶段 6：Humanizer 去机械感
   └─► 阶段 7：Final Edit 与移动端排版检查
        标题、摘要、封面、工作台与草稿箱按需处理
```
...
└── references/                       # 细分专业知识库 (Progressive Disclosure)
    ...
    ├── author_trace.md               # 作者痕迹检查与真实素材回填
    ├── humanizer.md                  # 责任编辑审校准则与全文检查线索
